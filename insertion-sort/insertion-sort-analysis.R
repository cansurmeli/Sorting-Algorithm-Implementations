...
ggplot(dataSet, aes(x = numberCount, y = secondsElapsed)) + 
	geom_point(aes(col = knownNumberArrangement), size = 3) +                                # graph a scatter plot
	geom_smooth(method = "lm", col = "steelblue", size = 1) +
	geom_count(col="tomato3") +		                                                # counts chart/ overcoming the problem of data points that overlap
	ggtitle("INSERTION SORT", subtitle = "Number Count vs. Seconds Elapsed") +
	xlab("Number Count") +
...
# ADDITIONAL
	# geom_text_repel(aes(label=numberCount), size = 2, data = dataSet)